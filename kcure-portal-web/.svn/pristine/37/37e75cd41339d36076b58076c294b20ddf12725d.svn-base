	...
	$(document).ready(function() {
		...
		gridView.onCellClicked = function (grid, clickData) {
			if(clickData.cellType != "header"){
		    	...
		    	$("#srchForm").attr("action", "/portal/bbs/qna/detailViewQna.do").submit();
			}
		}
		...
		searchDate = function() {
			RG.doQuery({
						action : "/portal/bbs/qna/selectQnaList.do",
						form : "srchForm",
						dataProviderPram : gridDataProvider,
						gridViewPram : gridView,
...
function fnRegist(){
	$("#srchForm").attr("action", "/portal/bbs/qna/insertQna.do").submit();
}
...
function fnPwCheck(){
	...
	if(wtrPswd != null && wtrPswd != ""){
		...
		$.ajax({
			url: "<c:url value='/portal/bbs/qna/qnaPwCheck.do'/>",
			type:"POST",
			data: data,
			dataType: "json",
	...
	<div class="inquiry_list_wrap">
		<div class="con">
			<div class="sub_pag_title">
				<p>문의하기</p>
				<span>공공데이터포털 이용에 관한 문의/답변 창구로서 공개를 원칙으로 운영합니다. 비공개를 원할 경우 마이페이지에서 확인하실 수 있습니다.<br>문의는 2일 이내 답변 드립니다.</span>
			</div>
			<div class="board_tab">
				<ul>
					<!-- class="on"일때 활성화 -->
					...
					<c:forEach var="list" items="${qstnClsSpcdList}" >
						<li><a href="javascript:void(0)" onclick="fnList('<c:out value="${list.detlCd}"/>');" class="${searchVO.searchKeyword eq list.detlCd ? 'on' : ''}" id="qstnClsSp_${list.detlCd}">${list.detlCdNm}</a></li>
					</c:forEach>
				</ul>
			</div>
