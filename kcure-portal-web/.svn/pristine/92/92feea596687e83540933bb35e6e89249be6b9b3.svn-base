...
<%
/* =================================================================
* @FileName : ViewComboInfReqData
* @date: 2023. 4. 14.
* @author : bhs
* @설명 : 데이터신청 > 자료요청(임상+공공 결합)
* =================================================================
* 수정일 작성자 내용
* -----------------------------------------------------------------------
* 2023. 4. 14. bhs 파일생성
...
<%@include file="/WEB-INF/jsp/kcure/portal/com/global_import.jsp"%>
								...
								<div class="top">
									...
									<div class="scroll">
										<c:forEach var="list" items="${treeList}" varStatus="rowInx">
											<c:if test="${list.itemLvl eq 1 and list.itemCd eq '01'}">
												<div class="public">
													<p class="sub">${list.itemNm1}</p>
													<ul>
														<c:forEach var="list2" items="${treeList}" varStatus="rowInx2">
															<c:if test="${list2.itemLvl eq 2 and list2.parentItemCd eq '01'}">
																<li>
											...
											<c:if test="${list.itemLvl eq 1 and list.itemCd eq '02'}">
												<div class="clinical">
													<p class="sub">${list.itemNm1}</p>
													<ul>
														<c:forEach var="list3" items="${treeList}" varStatus="rowInx3">
															<c:if test="${list3.itemLvl eq 2 and list3.parentItemCd eq '02'}">
																<li>
																	<a href="javascript:fnGetDtslist('${list3.parentItemCd}', '${list3.itemKey}', '${list3.itemCd}')" class="divPrtiItem" data1="${list3.parentItemCd}" data2="${list3.itemKey}" data3="${list3.itemNm2}">
																		${list3.itemNm1}
																	</a>
																</li>
															</c:if>
	...
	<form id="frmDownExcel">
		...
		<input type="hidden" id="selectDtsLclsCdVal" name="dtsLclsCd" />
    ...
    <form id="frmReqDts">
    	...
    	<input type="hidden" id="pblDtsSpcd" name="pblDtsSpcd"  value=""/>
...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		...
		$('.divPrtiItem').click(function() {
			if($("#dtapStatSpcd").val() == "0000000027"){
				return;
			}
	...
	* 데이터셋 리스트 가져오기
	*/
	function fnGetDtslist(gubun, prtiId, pblDtsSpcd){		
		if($("#dtapStatSpcd").val() == "0000000027"){
			return;
		}
		
		$("#selectedGubun").val(gubun);
		$("#pblDtsSpcd").val(pblDtsSpcd);
			...
			if(resultCnt > 0){
				...
				for(let i=0;i<resultCnt;i++){
					if(resultList[i].itemLvl == "1"){
						...
						html += '<label class="n_checkbox_s" onclick="fnEvent(' + i + ', ' + resultList[i].pblDtsSpcd + ')">'
						html += '<input type="checkbox" id="lnm_' + i + '" onclick="fnCheckDts1(' + i + ', ' + resultList[i].dtsLclsCd + ')" value="'+resultList[i].pblDtsSpcd+'">'
						...
						for(let k=0;k<resultCnt;k++){
							dtsLslCdVal =  resultList[k].dtsLclsCd;
				...
				setTimeout(() => {
					  if ($("#selectedGubun").val() == "01") {
						 ...
						 fnInitChcekbox();
						 fnSetCheckboxInitNew("임상", prtiId);
					  }
					}, 100);
			}
	      }
					...
					if(dtsKnmArray.length > 0){
						for(let k=0;k<dtsKnmArray.length;k++){
							if(resultList[i].gubun == "공공"){
								//fnAddGridHistoryData(resultList[i].gubun, resultList[i].pritnm, resultList[i].prtiid, resultList[i].pritnm, resultList[i].dtslclscd, dtsKnmArray[k], resultList[i].attfnm, resultList[i].attfseq, resultList[i].histdatayn);
	...
	* 데이터셋 대분류 체크박스 이벤트
	*/
	function fnEvent(id, pblCd){
		if($("#dtapStatSpcd").val() == "0000000027"){
			return;
		}
		
		var isDisabled = $("#lnm_" + id).prop("disabled");
		if (isDisabled) {
	...
	/*
	* 데이터셋 대분류 체크박스 이벤트 (id:checkbox id, pblCd: 데이터셋 대분류코드)
	*/
	function fnCheckDts1(id, pblCd){
		if($("#dtapStatSpcd").val() == "0000000027"){
			return;
		}
		...
		if($("#selectedGubun").val() == "01"){
			...
			var prtiNmVal = "국립암센터";
			var prtiIdVal = "M0019";
			var idVal1 = "lnm_" + id; //체크박스 식별용
			var idVal2 = "lnm_" + pblCd + "_" + id; //그리드데이터 저장용 (표준과 맞춤의 구분위함)
			
			//공공 데이터셋 체크상태인 경우
			if($("#" + idVal1).prop("checked")){
				if(id == "0") {
					if(pblCd == "06"){
						dtsNmVal = "암공공_표본_전체";
						dtsIdVal = "06";
				...
				fnAddGridSelectedPubData(dtsIdVal, idVal1, idVal1, gubunVal, gubunVal, prtiIdVal, prtiNmVal, dtsIdVal, dtsNmVal);
			}
			//체크해제인 경우
			else{
				var rowCnt = _dataProvider.getRowCount();
				...
				for(let k=0;k<rowCnt;k++){
					...
					let gubn = _dataProvider.getValue(k, "gubun");
					let dtsLclsCd = _dataProvider.getValue(k, "dtsLclsCd");

					if(gubn == "공공"){
						if(comp == idVal1 && dtsLclsCd == pblCd){
							if(_dataProvider.getValue(k, "isUpload") == "Y"){
								alert("해당 데이터는 이미 업로드처리된 상태입니다.\n업로드데이터를 제거 후 시도해주세요!");
								$("#"+idVal1).prop("checked", true);
								return;
							}else{
	...
	*/
	function fnDownloadExcel() {
		...
		$("#selectDtsLclsCdVal").val(dtsLclsCdVal);
	...
	* 데이터셋(공공) 선택영역 그리드 적용
	*/
	function fnAddGridSelectedPubData(dlCd, id1, id2, datas, data1, data2, data3, data4, data5) {
		var rowCnt = _dataProvider.getRowCount();
		...
		values = {dtsLclsCd: dlCd, checkId: checkIdVal, selectId: selectIdVal, gubun: gubunVal, prtiNm: prtiNmVal, prtiId: prtiIdVal, dtsId: dtsIdVal, dtsNm: dtsNmVal, histDataYn: "N"};
	...
	function fnInitPubChcekbox(){
		...
		if(rowCnt > 0){
			for(let i=0;i<rowCnt;i++){
				...
				checkIdVal = _dataProvider.getValue(i, "checkId");
				dtsIdVal = _dataProvider.getValue(i, "dtsId");

				if(gubunVal == "공공"){
					if($("#pblDtsSpcd").val() == "02"){
						if(dtsIdVal == "05"){
							$('input[type="checkbox"][value="02"]').prop('checked', true);
						}
					}else{
						if(dtsIdVal == "06"){
							$("#lnm_0").prop("checked", true);
						}else if(dtsIdVal == "07"){
							$("#lnm_1").prop("checked", true);
						}else if(dtsIdVal == "08"){
							$("#lnm_2").prop("checked", true);
						}else if(dtsIdVal == "09"){
							$("#lnm_3").prop("checked", true);
						}else if(dtsIdVal == "10"){
							$("#lnm_4").prop("checked", true);
						}else if(dtsIdVal == "11"){
							$("#lnm_5").prop("checked", true);
						}
					}
				}
			}
	...
	function fnInitChcekbox(){
		...
		if(rowCnt > 0){
			...
			for(let i=0;i<rowCnt;i++){
				...
				selectIdValTmp += "|";
				console.log("selectIdValTmp : " + selectIdValTmp);
			}
	...
	*/
	...
	/*
	* 임상 데이터셋 체크박스 disable적용 (이미 업로드적용된 임상기관의 데이터셋 체크박스는 disable적용)
	*/
 	function fnSetCheckboxInitNew(gubun, prtiId){
 		var rowCnt = _dataProvider.getRowCount();
 		...
 		for(let i=0;i<rowCnt;i++){
 			prtiIdVal = _dataProvider.getValue(i, "prtiId");
 			gubunVal =  _dataProvider.getValue(i, "gubun");
 			isUploadVal = _dataProvider.getValue(i, "isUpload");
 			
 			if(prtiId == prtiIdVal && gubun == gubunVal){
 				if(isUploadVal == "Y"){
