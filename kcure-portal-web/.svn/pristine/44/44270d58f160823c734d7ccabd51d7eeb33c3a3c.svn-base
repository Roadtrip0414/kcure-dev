...
function fnList(){
	$("#insertFrm").attr("action", "/portal/bbs/prt/qna/viewPrtQna.do").submit();
}
...
function fnSave(){
	...
	if(chk){
		...
		$.ajax({
			type:"POST",
			url: "/portal/bbs/prt/qna/saveQna.do",
			processData: false,
			contentType: false,
			dataType: "json",
			data,
			success: function(resp){
				console.log(resp);
				location.href = "/portal/bbs/prt/qna/viewPrtQna.do";
			},
			err: function(err){
				console.log("err:", err);
			}
               ...
               <div class="int_table">
                   <div class="top">
					...
					<form id="insertFrm" name="insertFrm" enctype="multipart/form-data" method="post">
						...
						<input type="hidden" name="instGnrSpcd" value="02"/><!-- 01: 일반, 02:기관 -->
						<input type="hidden" name="prtiId" value="<c:out value="${prtiId}"/>"/>
	                   <div class="item">
	                       <div class="int_list">
	                           <p class="title">이름</p>
	                           <ul>
	                       ...
	                       <div class="int_list">
	                           <p class="title">참여기관</p>
	                           <ul>
	                               <li><c:out value="${prtiNm}"/></li>
	                           </ul>
	                       </div>
	                       <div class="int_list">
