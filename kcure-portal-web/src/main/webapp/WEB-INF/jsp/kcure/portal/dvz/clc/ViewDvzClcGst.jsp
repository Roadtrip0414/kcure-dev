<%
/* =================================================================
 * @FileName : ViewDvzClcGst.jsp
 * @date: 2023.02.01
 * @author : kyh
 * @설명 : 데이터시각화 임상라이브러리 위암 화면
 * =================================================================
 * 수정일         작성자             내용     
 * -----------------------------------------------------------------------
 * 2023.02.01     kyh           파일생성
...
function goDvzClcGst(subVslzMenuId, frtVslzMenuId, srchVslzMenuId, smryStatsNo){
	...
	$("#listFrm").attr("action", "/portal/dvz/clc/gst/viewDvzClcGst.do").submit();
}
	...
	<div class="wrap">
		...
		<div class="sub_nav_wrap">
			<c:forEach items="${dvzClcTopMnuList}" var="list" varStatus="rowInx" >
				...
				<a onClick="javascript:goDvzClcGst('${list.vslzMenuId}', '', '', '')" class="<c:if test="${list.vslzMenuId eq searchVO.subVslzMenuId}">on</c:if>">${list.vslzMenuNm}</a>
			</c:forEach>
		</div>
									...
									<ul>
										<li>
											<c:if test="${list.childCnt eq 0}">
												<a onClick="javascript:goDvzClcGst('${searchVO.subVslzMenuId}', '', '${list.vslzMenuId}', '${list.smryStatsNo}')" class="<c:if test="${list.vslzMenuId eq searchVO.srchVslzMenuId}">active</c:if>" id="left_${list.vslzMenuId}">${list.vslzMenuNm}</a>
											</c:if>
												...
												<ul>
													<c:forEach items="${dvzClcLeftMnuList}" var="list2" begin="${rowInx.index+1}" varStatus="rowInx2" >
														<c:if test="${list2.depLv eq 2 and list2.hghrVslzMenuId eq list.vslzMenuId}">
															<li>
																<!-- class="on" 시 색변경 -->
																<a onClick="javascript:goDvzClcGst('${searchVO.subVslzMenuId}', '${list.vslzMenuId}', '${list2.vslzMenuId}', '${list2.smryStatsNo}')" class="<c:if test="${list2.vslzMenuId eq searchVO.srchVslzMenuId}">on</c:if>">${list2.vslzMenuNm}</a>
															</li>
														</c:if>
													</c:forEach>
												</ul>
