...
; (function ($w) {
    ...
    jm.prototype = {
        ...
        mouseover_handle: function (e) {
            ...
            if (!!nodeid){
				var target = this.get_node(nodeid );
				if(target.data && target.data.overEvent){
					target.data.overEvent(target);
				}
			}
        },
        ...
        click_handle: function (e) {
            ...
            if (isexpander) {
				...
				var nodeid = this.view.get_binded_nodeid(element);
			  	if (!!nodeid){
					var target = this.get_node(nodeid);
					if(target.data && target.data.clickEvent){
						target.data.clickEvent(target );
					}else{
						var the_node = this.get_node(nodeid);
						console.log(element,nodeid,the_node);
...
console.log("toggle_node-------------------------------->",node);
    ...
    jm.view_provider.prototype = {
        ...
        expand_size: function () {
			...
			//해당 div 높이 조절 할수 있도록 수정.
			$(this.container).css('height', this.size.h);
