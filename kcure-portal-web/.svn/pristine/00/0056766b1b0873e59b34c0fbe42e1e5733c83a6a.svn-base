...
public class CodeSelectTag_backup extends RequestContextAwareTag implements ServletContextAware{
	...
	protected int doStartTagInternal() throws Exception {
		...
		if (tagsService == null) { 
			WebApplicationContext wac = getRequestContext().getWebApplicationContext(); this.tagsService =wac.getBean(TagsService.class); 
		}
	  ...
	  private String buildSelectTag(List<ComCdDetailVo> commonCodeDetails) {
		  ...
		  builder.append(" name=\"").append(getName() != null ? getName() :
		  getId()).append("\""); if (attributes != null) {
		  builder.append(" ").append(attributes); } builder.append(">");
		  
		  if ("Y".equalsIgnoreCase(getDefaultYn())) {
		  builder.append("<option value=\"\">").append(getDefaultText()).append(
		  "</option>"); }
		  
		  if (commonCodeDetails != null) { for (ComCdDetailVo commonCodeDetail :
		  commonCodeDetails) { builder.append("<option value=\"");
