...
#boardUpload1 div.list {
	display: flex;
	column-gap: 10px;
}

#boardUpload1 input.upload {
	padding: 0 5px;
}

#boardUpload1iv div.list .cancel,
#boardUpload1 div.list .delete
{
	margin: 0 5px;
	padding: 0 5px;
}

#boardUpload1 div.list a {
	display: inline;
}
</style>
...
#boardUpload2 div.list {
	display: flex;
	column-gap: 10px;
}

#boardUpload2 input.upload {
	padding: 0 5px;
}

#boardUpload2iv div.list .cancel,
#boardUpload2 div.list .delete
{
	margin: 0 5px;
	padding: 0 5px;
}

#boardUpload2 div.list a {
	display: inline;
}
</style>
...
#boardUpload3 div.list {
	display: flex;
	flex-direction: column;
	column-gap: 10px;
}

#boardUpload3 input.upload {
	padding: 0 5px;
}

#boardUpload3iv div.list .cancel,
#boardUpload3 div.list .delete
{
	margin: 0 5px;
	padding: 0 5px;
}

#boardUpload3 div.list a {
	display: inline;
}
</style>
...
$(document).ready(() => {
	...
	const fu2 = FileUpload({
		...
		renderHeader: (uploadId) => {
			...
			<input type="button" id="` + uploadId + `Button" class="upload" value="Upload2" onclick="uploadButtonClick('` + uploadId + `')" />`;
		},
	...
	const fu3 = FileUpload({
		...
		listIdUpload: "boardUpload3ListUpload",
		listIdDownload: "boardUpload3ListDownload",
		url: "/portal/example/file/nonCheck/uploadFile.do",
		useZip: true,
		useDrop: true,
		maxNumberOfFiles: 0,
...
<form id="frm" method="POST" action="/file/nonCheck/uploadFile.do" enctype="multipart/form-data">
	...
	<div class="list upload" id="boardUpload3ListUpload"></div>
	<div class="list download" id="boardUpload3ListDownload"></div>
