...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		...
		$('.divPrtiItem').click(function() {
			if($("#dtapStatSpcd").val() == "0000000007"){
				return;
			}
		    $('li').removeClass('on');
					...
					if(dtsKnmArray.length > 0){
						for(let k=0;k<dtsKnmArray.length;k++){
							if(resultList[i].gubun == "공공"){
								fnAddGridHistoryData(resultList[i].gubun, resultList[i].pritnm, resultList[i].prtiid, resultList[i].pritnm, "02", dtsKnmArray[k], resultList[i].attfnm, resultList[i].attfseq, resultList[i].histdatayn);
							}else{
								fnAddGridHistoryData(resultList[i].gubun, resultList[i].pritnm, resultList[i].prtiid, resultList[i].pritnm, dtsEnmArray[k], dtsKnmArray[k], resultList[i].attfnm, resultList[i].attfseq, resultList[i].histdatayn);
							}
						}
					}
				}
	...
	* 히스토리 데이터셋 그리드 적용
	*/
	function fnAddGridHistoryData(gubun, prtiNm, prtiId, prtiNm, dtsId, dtsNm, attFnm, attfSeqVal, histDataYn) {
		...
		values = {selectId: idVal, gubun: gubunVal, prtiNm: prtiNmVal, prtiId: prtiIdVal, dtsId: dtsIdVal, dtsNm: dtsNmVal, isUpload: "Y", attfSeq: attfSeqVal, uploadFileNm: attFnm, histDataYn: histDataVal};
